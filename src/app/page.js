                    ...
                    {({ inView, ref }) => (
                      <div ref={ref} className="bg-stone-900 rounded-lg overflow-hidden relative group">
                        <Link href={`/photo/${photo.id}`}>
                          <Image
                            className={`w-full h-auto rounded-md object-cover transition-all duration-500 ease-in-out group-hover:scale-[1.05] transform-gpu ${!inView ? "animate-[pulse_2s_cubic-bezier(0.4,0,0.6,1)_infinite] opacity-0" : "opacity-100"}`}
                            width={colWidth}
